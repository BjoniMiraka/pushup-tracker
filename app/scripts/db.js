...
class PushupDatabase {
    ...
    addPushup(date, count, note = '') {
        try {
            this.db.run(
                'INSERT INTO pushups (date, count, note) VALUES (?, ?, ?)',
                [date, count, note]
            );
            this.save();
            return true;
    ...
    getStatistics() {
        try {
            ...
            // Total pushups
            const totalResult = this.db.exec('SELECT SUM(count) as total FROM pushups');
            ...
            const avgPerDay = dailyTotals.length > 0 
                ? Math.round(total / dailyTotals.length) 
                : 0;
    ...
    async importDatabase(file) {
        try {
            ...
            const tables = importedDb.exec("SELECT name FROM sqlite_master WHERE type='table' AND name='pushups'");
            if (tables.length === 0) {
